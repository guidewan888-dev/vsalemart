...
import { FlashSaleSection, ProductSection } from "@/components/home/product-sections";
import { CustomerReviews, EditorialBanner, HeroSection, InstitutionalBanner, OfferStrip, PromotionGrid, ShopByUseSection, CategoryGrid, TrustBenefits } from "@/components/home/sections";
...
export default async function HomePage() {
  ...
  const flashProducts = data.products.filter((product) => product.isFlashSale || (product.compareAtPrice && product.compareAtPrice > product.price)).slice(0, 4);
  const maxDiscount = Math.max(0, ...data.products.map((product) => product.compareAtPrice && product.compareAtPrice > product.price ? Math.round((1 - product.price / product.compareAtPrice) * 100) : 0));
  ...
  return <MotionProvider><CommerceProvider products={data.products} dataSource={data.source}>
    ...
    <main id="top" className="site-main">
      <HeroSection products={data.products} promotions={data.promotions}/><OfferStrip maxDiscount={maxDiscount}/>
      <FlashSaleSection products={flashProducts.length ? flashProducts : data.products.slice(0, 4)}/>
      <CategoryGrid categories={data.categories}/><PromotionGrid promotions={data.promotions}/>
      <div id="products" className="scroll-mt-28"><ProductSection id="best-sellers" title="ขายดีตอนนี้" subtitle="สินค้าที่ลูกค้าเลือกซื้อมากที่สุด" products={bestSellers}/></div>
      ...
      <InstitutionalBanner/><TrustBenefits/><CustomerReviews reviews={data.reviews}/><NewsletterSection/>
    </main>
