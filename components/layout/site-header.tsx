...
export function AnnouncementBar() { return <div className="overflow-hidden bg-[linear-gradient(90deg,#0868ff_0%,#7650ea_52%,#ff4f67_100%)] text-white"><div className="announcement-track flex min-h-8 w-max items-center gap-12 px-4 text-[11px] font-bold"><span>🎁 ส่งฟรีทุกออเดอร์ เมื่อซื้อครบ 499 บาท</span><span>⚡ โปรพร้อมช้อป · จัดส่งทั่วไทย</span><span>🎁 ส่งฟรีทุกออเดอร์ เมื่อซื้อครบ 499 บาท</span><span>⚡ โปรพร้อมช้อป · จัดส่งทั่วไทย</span></div></div>; }
...
export function SiteHeader({ products, categories }: { products: HomeProduct[]; categories: HomeCategory[] }) {
  ...
  return <header className={`sticky top-0 z-40 border-b border-[#e5edf9] bg-white/96 transition-shadow backdrop-blur ${scrolled ? "shadow-[0_8px_24px_rgba(35,67,118,.10)]" : ""}`}>
    <div className={`mx-auto flex max-w-[1320px] items-center gap-3 px-4 transition-[height] md:px-6 ${scrolled ? "h-14" : "h-16 md:h-[68px]"}`}>
      ...
      <div className="ml-auto flex items-center gap-1">
        ...
        <Link href="/account" className="hidden min-h-11 items-center gap-2 rounded-full px-3 text-sm font-bold text-[#17355e] hover:bg-[#edf5ff] md:flex"><UserRound className="size-5 text-[#0872f5]"/><span>เข้าสู่ระบบ</span></Link>
        <button className="relative hidden size-11 place-items-center rounded-full text-[#17355e] hover:bg-[#fff0f3] hover:text-[#ff4f67] md:grid" aria-label={`รายการโปรด ${favorites.size} รายการ`}><Heart className="size-5"/>{favorites.size > 0 && <Badge count={favorites.size}/>}</button>
        <button className="relative grid size-11 place-items-center rounded-full bg-[#edf5ff] text-[#0872f5] hover:bg-[#0872f5] hover:text-white" onClick={(event) => openCart(event.currentTarget)} aria-label={`เปิดตะกร้า มี ${cartCount} รายการ`}><ShoppingCart className="size-5"/><Badge count={cartCount}/></button>
      </div>
    </div>
...
function Logo() { return <Link href="/" className="flex shrink-0 items-center gap-2 text-[#0872f5]" aria-label="V SALE หน้าแรก"><span className="grid size-10 -rotate-3 place-items-center rounded-xl bg-[#0872f5] text-[27px] font-black italic leading-none text-white shadow-[4px_4px_0_#ffd83d]">V</span><span className="text-lg font-black tracking-tight">SALE<small className="block text-[6px] font-bold tracking-[.14em] text-[#0b1533]">GOOD TOOLS · GREAT DAYS</small></span></Link>; }
...
function DesktopNav({ categories }: { categories: HomeCategory[] }) { return <nav className="group/nav hidden border-t border-[#edf0f5] md:block" aria-label="เมนูหลัก"><div className="mx-auto flex h-10 max-w-[1200px] items-center justify-center gap-9 px-6 text-[13px] font-bold text-[#17355e]"><Link className="text-[#0872f5]" href="/">หน้าแรก</Link><Link href="#products">สินค้าทั้งหมด</Link><Link className="inline-flex items-center gap-1 text-[#ef3755]" href="#promotions"><span className="size-1.5 rounded-full bg-[#ef3755]"/>โปรโมชั่น</Link><Link href="#best-sellers">สินค้าขายดี</Link><Link href="#new-arrivals">สินค้าใหม่</Link><div className="group relative h-full"><button className="h-full font-bold">หมวดหมู่</button><div className="invisible absolute right-[-5rem] top-full z-50 w-[min(760px,calc(100vw-2rem))] rounded-b-2xl border border-[#e1e6ee] bg-white p-5 opacity-0 shadow-xl transition group-hover:visible group-hover:opacity-100 group-focus-within:visible group-focus-within:opacity-100"><div className="grid grid-cols-3 gap-2">{categories.map((item) => <Link key={item.id} href={`#category-${item.slug}`} className="rounded-xl px-3 py-3 hover:bg-[#eef4ff]">{item.name}</Link>)}</div></div></div></div></nav>; }
