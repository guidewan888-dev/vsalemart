...
export function AnnouncementBar() { return <div className="overflow-hidden bg-[#082653] text-white"><div className="announcement-track flex min-h-7 w-max items-center gap-12 px-4 text-[11px]"><span>ส่งฟรีทุกออเดอร์ เมื่อซื้อครบ 499 บาท</span><span>จัดส่งทั่วไทย</span><span>ส่งฟรีทุกออเดอร์ เมื่อซื้อครบ 499 บาท</span><span>จัดส่งทั่วไทย</span></div></div>; }
...
function Logo() { return <Link href="/" className="flex shrink-0 items-center gap-1.5 text-[#0872f5]" aria-label="V SALE หน้าแรก"><span className="text-[29px] font-black italic leading-none">V</span><span className="text-lg font-black tracking-tight">SALE<small className="block text-[6px] font-bold tracking-[.14em] text-[#0b1533]">GOOD TOOLS · GREAT DAYS</small></span></Link>; }
...
function DesktopNav({ categories }: { categories: HomeCategory[] }) { return <nav className="group/nav hidden border-t border-[#edf0f5] md:block" aria-label="เมนูหลัก"><div className="mx-auto flex h-10 max-w-[1320px] items-center justify-center gap-9 px-6 text-[13px] font-semibold"><Link className="text-[#0872f5]" href="/">หน้าแรก</Link><Link href="#products">สินค้าทั้งหมด</Link><Link href="#promotions">โปรโมชั่น</Link><Link href="#best-sellers">สินค้าขายดี</Link><Link href="#new-arrivals">สินค้าใหม่</Link><div className="group relative h-full"><button className="h-full">หมวดหมู่</button><div className="invisible absolute right-[-5rem] top-full z-50 w-[min(760px,calc(100vw-2rem))] rounded-b-2xl border border-[#e1e6ee] bg-white p-5 opacity-0 shadow-xl transition group-hover:visible group-hover:opacity-100 group-focus-within:visible group-focus-within:opacity-100"><div className="grid grid-cols-3 gap-2">{categories.map((item) => <Link key={item.id} href={`#category-${item.slug}`} className="rounded-xl px-3 py-3 hover:bg-[#eef4ff]">{item.name}</Link>)}</div></div></div></div></nav>; }
