...
function DesktopNav({ categories }: { categories: HomeCategory[] }) { return <nav className="group/nav hidden border-t border-[#edf0f5] md:block" aria-label="เมนูหลัก"><div className="mx-auto flex h-11 max-w-[1440px] items-center justify-center gap-9 px-6 text-sm font-semibold"><Link className="text-[#146ef5]" href="/">หน้าแรก</Link><Link href="#products">สินค้าทั้งหมด</Link><Link href="#promotions">โปรโมชั่น</Link><Link href="#best-sellers">สินค้าขายดี</Link><Link href="#new-arrivals">สินค้าใหม่</Link><div className="group relative h-full"><button className="h-full">หมวดหมู่</button><div className="invisible absolute left-1/2 top-full z-50 w-[760px] -translate-x-1/2 rounded-b-2xl border border-[#e1e6ee] bg-white p-5 opacity-0 shadow-xl transition group-hover:visible group-hover:opacity-100 group-focus-within:visible group-focus-within:opacity-100"><div className="grid grid-cols-3 gap-2">{categories.map((item) => <Link key={item.id} href={`#category-${item.slug}`} className="rounded-xl px-3 py-3 hover:bg-[#eef4ff]">{item.name}</Link>)}</div></div></div></div></nav>; }
