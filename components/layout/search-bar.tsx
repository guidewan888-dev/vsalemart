...
export function SearchBar({ products, categories, compact = false, onSelect }: { products: HomeProduct[]; categories: HomeCategory[]; compact?: boolean; onSelect?: () => void }) {
  ...
  return <div className={`relative ${compact ? "w-full" : "hidden w-full md:block"}`}>
    <label className="flex min-h-12 items-center gap-3 rounded-xl border border-[#d9e1ed] bg-[#f7f9fc] px-4 focus-within:border-[#146ef5] focus-within:ring-2 focus-within:ring-[#146ef5]/15"><Search className="size-5 shrink-0 text-[#637089]"/><span className="sr-only">ค้นหาสินค้า</span><input ref={inputRef} value={query} onChange={(event) => { setQuery(event.target.value); setOpen(true); }} onFocus={() => setOpen(true)} onKeyDown={onKeyDown} placeholder="ค้นหาสินค้า ชื่อหนังสือ ระดับชั้น หรือรหัสสินค้า" role="combobox" aria-expanded={open} aria-controls="search-results" aria-activedescendant={results[active] ? `search-result-${results[active].id}` : undefined} className="min-w-0 flex-1 bg-transparent text-base outline-none placeholder:text-[#7b8495]"/><button type="button" className={`grid size-10 place-items-center ${query ? "" : "invisible"}`} onClick={() => { setQuery(""); inputRef.current?.focus(); }} aria-label="ล้างคำค้น"><X className="size-4"/></button></label>
