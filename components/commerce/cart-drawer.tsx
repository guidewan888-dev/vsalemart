...
export function CartDrawer() {
  ...
  return <AnimatePresence>{cartOpen && <>
    ...
    <motion.aside role="dialog" aria-modal="true" aria-labelledby="cart-title" className="fixed inset-y-0 right-0 z-[60] flex w-full max-w-md flex-col bg-white shadow-2xl" initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ duration: .26, ease: [.22,1,.36,1] }}>
      <header className="flex items-center justify-between border-b border-[#dce8f6] bg-[linear-gradient(100deg,#e5f4ff_0%,#f1eaff_55%,#ffe6eb_100%)] px-5 py-5"><div><p className="text-xs font-black text-[#0872f5]">ตะกร้าของคุณ</p><h2 id="cart-title" className="text-2xl font-black text-[#0b1533]">รายการสินค้า</h2></div><button ref={closeButton} onClick={closeCart} className="grid size-11 place-items-center rounded-full bg-white/75 hover:bg-white" aria-label="ปิดตะกร้า"><X/></button></header>
      {cart.length === 0 ? <div className="grid flex-1 place-content-center px-8 text-center"><ShoppingBag className="mx-auto size-10 text-[#146ef5]"/><p className="mt-4 text-xl font-bold">ตะกร้ายังว่างอยู่</p><p className="mt-2 text-sm text-[#667085]">เลือกสินค้าที่ต้องการ แล้วกดเครื่องหมายบวกเพื่อเพิ่มลงตะกร้า</p><button onClick={closeCart} className="mt-6 min-h-12 rounded-full bg-[#146ef5] px-6 font-bold text-white">เลือกซื้อสินค้า</button></div> : <>
        ...
        <footer className="border-t border-[#e1e6ee] bg-[#f8fbff] p-5"><div className="flex justify-between"><span className="font-bold">ยอดรวม</span><strong className="text-xl text-[#f23856]">{money.format(total)}</strong></div><p className="mt-2 text-xs text-[#667085]">ตรวจสอบสินค้าและชำระเงินผ่านร้าน V SALE บน Shopee</p><a href="https://shopee.co.th/vsale?entryPoint=ShopBySearch&searchKeyword=vsale" target="_blank" rel="noreferrer" className="mt-4 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-full bg-[#0872f5] font-black text-white shadow-[0_8px_20px_rgba(8,114,245,.2)]">ไปสั่งซื้อที่ Shopee <ExternalLink className="size-4"/></a><p className="mt-2 text-center text-[10px] text-[#7a8799]">รายการในตะกร้านี้ใช้ช่วยจดสินค้า กรุณาเลือกสินค้าอีกครั้งใน Shopee</p></footer>
      </>}
    </motion.aside>
  </>}</AnimatePresence>;
}
