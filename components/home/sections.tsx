...
const promotionTones = [
  "from-[#0868ff]/95 via-[#1686ff]/72 to-[#35c5ff]/20",
  "from-[#ff415f]/95 via-[#ff7256]/72 to-[#ffcb3d]/15",
  "from-[#7047eb]/92 via-[#8d62f0]/66 to-transparent",
  "from-[#ff8a00]/92 via-[#ffb800]/62 to-transparent",
];
...
export function HeroSection({ products, promotions }: { products: HomeProduct[]; promotions: HomePromotion[] }) {
  ...
  return <section className="px-4 pb-3 pt-4 md:px-6 md:pb-4 md:pt-5">
    <div className="mx-auto grid max-w-[1200px] gap-3 lg:grid-cols-[minmax(0,1.72fr)_minmax(300px,.68fr)]">
      <div className="hero-campaign relative min-h-[500px] overflow-hidden rounded-[26px] bg-[#0872f5] shadow-[0_18px_50px_rgba(17,103,226,.2)] md:min-h-[430px]">
        ...
        <div className="absolute inset-0 bg-[linear-gradient(90deg,#0868ff_0%,rgba(8,104,255,.96)_38%,rgba(8,104,255,.50)_62%,rgba(8,104,255,.04)_100%)] max-md:bg-[linear-gradient(180deg,#0868ff_0%,rgba(8,104,255,.97)_49%,rgba(8,104,255,.20)_100%)]"/>
        <span className="absolute -right-14 -top-16 size-52 rounded-full border-[34px] border-[#ffd83d]/90 md:size-64" aria-hidden="true"/>
        <span className="absolute bottom-6 right-6 hidden rotate-3 rounded-2xl bg-[#ff4f67] px-4 py-3 text-center text-white shadow-lg md:block"><b className="block text-3xl font-black leading-none">SALE</b><small className="font-bold">ดีลพร้อมช้อป</small></span>
        <div className="relative z-10 flex min-h-[500px] max-w-[570px] flex-col px-6 py-8 text-white md:min-h-[430px] md:justify-center md:px-11 md:py-9">
          <span className="inline-flex w-fit items-center gap-1.5 rounded-full bg-[#ffd83d] px-3 py-1.5 text-xs font-extrabold text-[#15315c]"><Sparkles className="size-3.5"/> โปรเด่นจาก V SALE</span>
          ...
          <div className="mt-5 flex flex-wrap items-center gap-2.5">
            <Link href="#flash-sale" className="inline-flex min-h-11 items-center gap-2 rounded-full bg-[#ffd83d] px-6 text-sm font-extrabold text-[#0c2b58] shadow-[0_9px_22px_rgba(255,216,61,.3)]">ช้อปโปรตอนนี้ <ArrowRight className="size-4"/></Link>
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-1">
        {sidePromotions.map((promotion, index) => <Link key={promotion.id} href={promotion.href} className={`group relative min-h-[190px] overflow-hidden rounded-[22px] ${index === 0 ? "bg-[#ffdf4d]" : "bg-[#ff6d79]"}`}>
...
export function OfferStrip({ maxDiscount }: { maxDiscount: number }) {
  const items = [
    { icon: <Truck/>, label: "ส่งฟรี", detail: "เมื่อครบ ฿499", tone: "bg-[#dff7ff] text-[#0067d8]" },
    { icon: <BadgePercent/>, label: maxDiscount > 0 ? `ลดสูงสุด ${maxDiscount}%` : "โปรราคาพิเศษ", detail: "สินค้าร่วมรายการ", tone: "bg-[#ffe2e7] text-[#e92f50]" },
    { icon: <Gift/>, label: "โปรพร้อมช้อป", detail: "เลือกดีลได้ทันที", tone: "bg-[#fff1b8] text-[#d66b00]" },
    { icon: <ShieldCheck/>, label: "ช้อปมั่นใจ", detail: "ตรวจสอบก่อนจัดส่ง", tone: "bg-[#dcf8ec] text-[#00895f]" },
  ];
...
export function SectionHeading({ title, linkLabel, href = "#products", copy, accent = "blue" }: { eyebrow?: string; title: string; linkLabel?: string; href?: string; copy?: string; accent?: "blue" | "coral" | "mint" | "purple" }) {
  const accents = { blue: "bg-[#0872f5]", coral: "bg-[#ff4f67]", mint: "bg-[#16bd8a]", purple: "bg-[#7b53ed]" };
  return <div className="mb-4 flex items-end justify-between gap-3"><div className="min-w-0"><div className="flex items-center gap-2"><span className={`h-7 w-1.5 rounded-full ${accents[accent]}`}/><h2 className="text-[clamp(1.65rem,2.8vw,2.2rem)] font-black leading-tight tracking-[-.035em] text-[#092653]">{title}</h2></div>{copy && <p className="mt-1 pl-3.5 text-xs text-[#60718b] sm:text-sm">{copy}</p>}</div>{linkLabel && <Link href={href} className="hidden shrink-0 items-center gap-1.5 rounded-full bg-[#edf5ff] px-3 py-2 text-xs font-extrabold text-[#0872f5] sm:flex">{linkLabel}<ArrowRight className="size-4"/></Link>}</div>;
}

export function CategoryGrid({ categories }: { categories: HomeCategory[] }) {
  const tones = ["#ddf2ff", "#fff0b9", "#ffe0e6", "#dff8e8", "#ede3ff", "#dfe8ff", "#ffe7d5", "#d8f7f4", "#e5edff"];
...
export function TrustBenefits() {
  ...
  return <section aria-label="ข้อมูลร้าน" className="mx-auto max-w-[1200px] px-4 py-3 md:px-0"><div className="grid overflow-hidden rounded-2xl border border-[#cfe1f8] bg-white shadow-[0_8px_24px_rgba(34,73,128,.06)] sm:grid-cols-3 sm:divide-x sm:divide-[#dbe8f7]">{items.map((item) => <div key={item.title} className="flex items-center justify-center gap-3 px-4 py-3"><span className="grid size-10 shrink-0 place-items-center rounded-full bg-[#e7f3ff] text-[#0872f5] [&_svg]:size-5">{item.icon}</span><span><b className="block text-sm text-[#082653]">{item.title}</b><small className="block text-[10px] text-[#667a96]">{item.detail}</small></span></div>)}</div></section>;
}

export function PromotionGrid({ promotions }: { promotions: HomePromotion[] }) {
  return <Reveal><section id="promotions" className="compact-section mx-auto max-w-[1200px] px-4 md:px-0"><SectionHeading title="โปรโมชั่นเลือกไว้ให้แล้ว" copy="ดีลเด่นสำหรับวันเรียน วันสอน และวันทำงาน" linkLabel="ดูสินค้าราคาพิเศษ" href="#flash-sale" accent="coral"/><div className="grid grid-cols-2 gap-3 lg:grid-cols-4">{promotions.slice(0, 4).map((promotion, index) => <Link key={promotion.id} href={promotion.href} className="group relative min-h-[220px] overflow-hidden rounded-[22px] shadow-[0_10px_25px_rgba(37,64,111,.10)] md:min-h-[245px]"><Image src={promotion.image.src} alt={promotion.image.alt} fill sizes="(max-width: 767px) 50vw, 300px" className="object-cover transition-transform duration-500 group-hover:scale-[1.04]"/><span className={`absolute inset-0 bg-gradient-to-t ${promotionTones[index]}`}/><span className="absolute left-3 top-3 rounded-full bg-[#fff06a] px-2.5 py-1 text-[10px] font-black text-[#18335b]">{index === 0 ? "โปรแนะนำ" : index === 1 ? "ลดแรง" : index === 2 ? "ชุดคุ้ม" : "ราคาเบา ๆ"}</span><span className="absolute inset-x-0 bottom-0 p-4 text-white"><small className="text-[10px] font-semibold text-white/85">{promotion.subtitle}</small><strong className="mt-1 flex items-center justify-between text-base font-black md:text-xl">{promotion.title}<span className="grid size-8 place-items-center rounded-full bg-white text-[#0868ff]"><ArrowRight className="size-4"/></span></strong></span></Link>)}</div></section></Reveal>;
}

export function EditorialBanner() {
  return <Reveal><section className="mx-auto max-w-[1200px] px-4 py-4 md:px-0"><div className="relative min-h-[300px] overflow-hidden rounded-[24px] bg-[#dff4ff] md:min-h-[245px]"><Image src="/images/vsale/banners/editorial-desk.png" alt="โต๊ะทำงานที่จัดอย่างเป็นระเบียบพร้อมแฟ้มและเครื่องเขียน" fill sizes="(max-width: 767px) 100vw, 1200px" className="object-cover object-[66%_center]"/><div className="absolute inset-0 bg-gradient-to-r from-[#dff4ff] via-[#dff4ff]/94 to-transparent"/><span className="absolute -left-10 -top-12 size-36 rounded-full bg-[#fff060]/65"/><div className="absolute inset-y-0 left-0 flex max-w-lg flex-col justify-center p-6 md:p-9"><span className="w-fit rounded-full bg-white px-3 py-1 text-[10px] font-extrabold text-[#0872f5]">ไอเดียจัดโต๊ะ</span><h2 className="mt-3 text-[clamp(2rem,3.4vw,3rem)] font-black leading-[1.02] text-[#082653]">จัดโต๊ะใหม่<br/><span className="text-[#0872f5]">ให้ไอเดียไปต่อ</span></h2><p className="mt-2 max-w-sm text-sm leading-6 text-[#405b7e]">อุปกรณ์สำนักงาน เอกสาร เทป และกาว ครบในที่เดียว</p><Link href="#bundles" className="mt-4 inline-flex min-h-10 w-fit items-center gap-2 rounded-full bg-[#ff4f67] px-5 text-sm font-extrabold text-white">ช้อปเซตสำนักงาน <ArrowRight className="size-4"/></Link></div></div></section></Reveal>;
}

const useCases = [["วันเรียน", "อุปกรณ์ครบ พร้อมลุยทุกวิชา", "use-student.png", "#0872f5"], ["วันสอน", "สื่อการสอน เอกสาร และอุปกรณ์", "use-teacher.png", "#7b53ed"], ["วันทำงาน", "จัดระเบียบงาน ให้สำเร็จทุกชิ้น", "use-office.png", "#16a87c"], ["วันสร้างสรรค์", "ศิลปะและงานฝีมือ จุดไอเดียใหม่", "use-creative.png", "#ff5365"]];
...
export function InstitutionalBanner() {
  return <Reveal><section id="institutional" className="mx-auto max-w-[1200px] px-4 py-4 md:px-0"><div className="relative min-h-[300px] overflow-hidden rounded-[24px] bg-[#e9e0ff] md:min-h-[215px]"><Image src="/images/vsale/banners/institutional-orders.png" alt="ห้องเรียนพร้อมกล่องอุปกรณ์สำหรับจัดส่งจำนวนมาก" fill sizes="(max-width: 767px) 100vw, 1200px" className="object-cover object-center"/><div className="absolute inset-0 bg-gradient-to-r from-[#7047eb] via-[#7047eb]/94 to-[#7047eb]/8"/><div className="absolute inset-y-0 left-0 flex max-w-xl flex-col justify-center p-6 text-white md:p-9"><span className="w-fit rounded-full bg-[#fff06a] px-3 py-1 text-[10px] font-black text-[#15315c]">สำหรับองค์กร</span><h2 className="mt-3 text-[clamp(1.8rem,3vw,2.5rem)] font-black leading-tight">สั่งยกชุด ยิ่งคุ้มกว่า</h2><p className="mt-2 max-w-md text-sm leading-6 text-white/88">สำหรับโรงเรียนและสำนักงาน ร้านช่วยตรวจจำนวนและจัดข้อเสนอให้เหมาะกับการใช้งาน</p><Link href="#contact" className="mt-4 inline-flex min-h-10 w-fit items-center gap-2 rounded-full bg-white px-5 text-sm font-extrabold text-[#6037d3]">สอบถามการสั่งซื้อ <ArrowRight className="size-4"/></Link></div></div></section></Reveal>;
}

export function CustomerReviews({ reviews }: { reviews: HomeReview[] }) {
  if (!reviews.length) return null;
  return <Reveal><section className="compact-section mx-auto max-w-[1200px] px-4 md:px-0"><SectionHeading title="ลูกค้าช้อปแล้วบอกต่อ" accent="purple"/><div className="grid gap-3 md:grid-cols-3">{reviews.map((review, index) => <article key={review.id} className={`rounded-[20px] p-4 ${index === 0 ? "bg-[#e5f4ff]" : index === 1 ? "bg-[#fff0c2]" : "bg-[#ffe3e8]"}`}><div className="flex items-center gap-3"><span className="grid size-9 place-items-center rounded-full bg-white text-sm font-black text-[#0872f5]">{review.authorInitial}</span><span><span className="block text-xs text-[#f4a600]">{"★".repeat(review.rating)}</span>{review.verified && <small className="text-[10px] font-bold text-[#16855b]">ซื้อแล้ว ✓</small>}</span></div><p className="mt-2 line-clamp-2 text-xs leading-5 text-[#405b7e]">“{review.excerpt}”</p></article>)}</div></section></Reveal>;
}
